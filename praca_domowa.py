...
for i in range(ilosc_elementow):
    ...
    if ilosc_kg_w_paczke + waga_kg > 20:
            ...
            print(f"Paczka {numer_aktusnej_paczki} zapelniona: {ilosc_kg_w_paczke} kg, pozostaly puste kg : {puste_kg_tej_paczki} ")
            summa_pustych_kg += puste_kg_tej_paczki
            if puste_kg_tej_paczki > najwiecej_pustych_kg :
                najwiecej_pustych_kg = puste_kg_tej_paczki
                numer_paczki_max_puste_kg = numer_aktusnej_paczki
            numer_aktusnej_paczki+=1
...
print (f"Paczka {numer_aktusnej_paczki} zapelniona: {ilosc_kg_w_paczke} kg, pozostaly puste: {puste_kg_ostatniej_paczki} ")
if puste_kg_ostatniej_paczki> najwiecej_pustych_kg :
    najwiecej_pustych_kg = puste_kg_ostatniej_paczki
    numer_paczki_max_puste_kg = numer_aktusnej_paczki

print()
print (f"Wyslano : {numer_aktusnej_paczki} paczek")
